...
// try-catch 문은 이 규칙에서 예외다. 변수를 초기화하는 표현식에서 검사 예외를 던질 가능성이 있다면 
// try 블록 안에서 초기화해야 한다.(그렇지 않으면 예외가 블록을 넘어 메서드에까지 전파된다)
// 한편, 변수 값을 try 블록 바깥에서도 사용해야 한다면 (비록 정확히 초기화하진 못하더라도) try 블록 앞에서 선언해야 한다.

// 반복문은 독특한 방식으로 변수 범위를 최소화해준다.

// 반복 변수의 값을 반복문이 종료된 뒤에도 써야 하는 상황이 아니라면 while 문 보다는 for문을 쓰는 편이 낫다.
public class Mehtod {
    ...
    public void test() {
        ...
        // 다음의 두 while 문을 보면 앞서의 for 문이 더 나은 이유를 알 수 있다. 다음 코드에는 버그도 하나 숨어 있다.
        ...
        // 하지만 두 번째 while 문은 c2 순회하 않고 곧장 끝나 버려 c2가 비었다고 착각하게 만든다.
        ...
        // 두 번째 while 문에는 복사해 붙여넣기 오류가 있다. 새로운 반복 변수 i2를 초기화 했지만,
        // 실수로 이전 while 문에서 쓴 i를 다시 쓴 것이다.


        // for-each를 포함한 for문을 사용하면 이런 복사해 붙여넣기 오류를 컴파일 타임에 잡아준다.
